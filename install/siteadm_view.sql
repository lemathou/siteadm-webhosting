...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `proftpd_user` AS select if (`account`.`id` IS NULL, 0, `account`.`id`) AS `account_id`,((`account`.`actif` OR `account`.`actif` IS NULL) and `ftp_user`.`actif`) AS `actif`,`ftp_user`.`id` AS `id`,CONCAT(if(`account`.`name` is null, 'common', `account`.`name`), '_', `ftp_user`.`username`) AS `username`,encrypt(`ftp_user`.`password`) AS `password`,(2000 + if(`account`.`id` IS NULL, 0, `account`.`id`)) AS `uid`,(2000 + if(`account`.`id` IS NULL, 0, `account`.`id`)) AS `gid`,concat('/home/siteadm/',if(`account`.`id` IS NULL, 'common', `account`.`folder`),'/',`ftp_user`.`type`,'/',`ftp_user`.`folder`) AS `folder`,'/bin/bash' AS `shell` from (`ftp_user` left join `account`  on((`account`.`id` = `ftp_user`.`account_id`)));
