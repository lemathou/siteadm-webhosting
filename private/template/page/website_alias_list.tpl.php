...
<tr>
	...
	<td><a href="website.php?alias_id=<?=$row["id"]?>"><?=$row["alias_name"]?>.<?=$domain->name?></a></td>
	<td><?php if ($row["website_id"]) { ?><a href="website.php?id=<?=$row["website_id"]?>"><?=$row["website_name"]?></a><?php } ?></td>
	<td><?php echo ($row["website_redirect"]) ?"OUI" : "NON"; ?></td>
...
{
?>
<p><select name="_website_alias_action" onchange="if (this.value && confirm('Êtes-vous certain ?')) this.form.submit(); else this.selectedIndex = 0;">
