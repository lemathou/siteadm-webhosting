...
?>
<tr>
	<td><a href="?account_id=<?php echo $row["account_id"]; ?>&app_id=<?=$row["id"]?>"><?=$row["name"]?></a></td>
	<td><?php if ($row["account_id"]) {  echo account($row["account_id"])->link(); } else { echo "<i>Partagé</i>"; } ?></td>
	<td><?php if ($row["language_bin_id"]) { echo language_bin($row["language_bin_id"]); } ?></td>
	<td align="right"><?=$row["apc_shm_size"]?> M</td>
	<td align="right"><?=$row["phppool_nb"]?></td>
	<td align="right"><?php echo ($row["worker_nb"])?$row["worker_nb"]:"0"; ?></td>
