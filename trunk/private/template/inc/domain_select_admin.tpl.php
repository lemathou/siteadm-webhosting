<form method="get">
	<p>
		Compte : <select name="account_id" onchange="this.form.submit();"><option></option>
			<?php
		...
		Domaine : <select name="domain_id" onchange="this.form.submit()"><option></option>
			<?php
			$query_string = "SELECT t1.id, t1.name FROM `domain` as t1 LEFT JOIN account as t2 ON t1.account_id=t2.id ORDER BY `name`";
			$query = mysql_query($query_string);
			while($row=mysql_fetch_assoc($query))
			{
				if (isset($_GET["domain_id"]) && $row["id"] == $_GET["domain_id"])
