<form method="post" action="<?php echo $website_alias->url(); ?>">
	...
	<input type="hidden" name="id"
		value="<?php echo $website_alias->id; ?>" />
	<?php } else { ?>
	<input type="hidden" name="domain_id"
		value="<?php echo $website_alias->domain_id; ?>" />
	<?php } ?>
	...
	<div style="width: 600px;" class="cadre">
		<table cellspacing="0" cellpadding="0" border="0" width="100%">
			<tr>
				<td>Sous-domaine :</td>
				<td><input name="alias_name"
					value="<?php echo $website_alias->alias_name; ?>" />.<?php if ($domain) echo $domain->name; else echo "*"; ?>
				</td>
			</tr>
			<tr>
				<td>Alias de :</td>
				<td><select name="website_id"><option value="0">-- Choisir au besoin
							--</option>
						<?
						...
						if ($d)
							echo "</optgroup>\n";
						?></select> <input type="checkbox" name="website_redirect"
					value="1"
					<?php echo ($website_alias->website_redirect) ?" checked" : ""; ?> />
					Rediriger vers l'alias</td>
			</tr>
			<tr>
				<td>Redirection vers :<br />(url complète)
				</td>
				<td><input name="redirect_url" size="64"
					value="<?php echo $website_alias->redirect_url; ?>" />
				</td>
			</tr>
		</table>
	</div>
