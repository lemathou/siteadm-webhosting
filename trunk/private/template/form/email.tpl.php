...
<form method="post" action="?domain_id=<?php echo $domain->id; ?>">
	...
	<p>
		<input type="submit" name="_email_update"
		...
		<table cellspacing="0" cellpadding="0" border="0">
			<tr>
				...
				<td class="field"><input name="name"
					...
					<?php if ($email->id) echo " readonly"; ?> /> @ <?=$domain->name?>
				</td>
			</tr>
			<tr>
				<td class="label">Boite active :</td>
				<td class="field"><input type="radio" name="actif" value="1"
				<?php if ($email->actif) echo " checked"; ?> /> OUI <input
					type="radio" name="actif" value="0"
					<?php if (!$email->actif) echo " checked"; ?> /> NON</td>
			</tr>
			<?php if (!$email->id) { ?>
			<tr>
