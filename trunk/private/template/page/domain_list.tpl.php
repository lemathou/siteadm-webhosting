<form method="post">
	<table cellspacing="2" cellpadding="2" border="1">
		...
		<tr class="colname">
			...
			<td rowspan="2" valign="top">Date de<br />renouvellement
			</td>
		...
		<tr>
			...
			<td><a href="?id=<?=$row["id"]?>"><?=$row["name"]?>
			</a>
			</td>
			<td><?=$row["renew_date"]?>
			</td>
			<td><a href="account.php?id=<?=$row["account_id"]?>"><?=$row["account_name"]?>
			</a>
			</td>
			<td><a href="website.php?domain_id=<?=$row["id"]?>">WEBSITES</a>
			</td>
			<td align="right"><? if ($row["website_nb"]) echo $row["website_nb"]; else echo "<span style=\"color:red;\">0</span>"; ?>
			</td>
			<td align="right"><?=$row["website_alias_nb"]?>
			</td>
			<td><a href="email.php?domain_id=<?=$row["id"]?>">EMAILS</a>
			</td>
			<td><?php if ($row["email_actif"]) echo "ACTIF"; else echo "<span style=\"color:red;\">INACTIF</span>"; ?>
			</td>
			<td align="right"><? if ($row["email_nb"]) echo $row["email_nb"]; else echo "<span style=\"color:red;\">0</span>"; ?>
			</td>
			<td align="right"><?=$row["email_alias_nb"]?>
			</td>
		</tr>
		<?php
		}
