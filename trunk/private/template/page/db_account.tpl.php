...
<form method="post">
	...
	<table cellspacing="2" cellpadding="2" border="1">
		...
		<tr class="mail">
			<td class="noborder"><input type="checkbox" name="_list_id[]"
				value="<?=$row["id"]?>" /></td>
			<td><a
				href="?account_id=<?php echo $row[account_id]; ?>&id=<?php echo $row["id"]; ?>"><?=$row["dbname"]?>
			</a></td>
			<td><?=$row["username"]?></td>
			<td><?=$row["password"]?></td>
			<td class="delete noborder"><a
				href="?account_id=<?=$account->id?>&db_del_id=<?=$row["id"]?>"
				onclick="return(confirm('Êtes-vous certain de vouloir supprimer cette base de donnée ?'))">X</a>
			</td>
		</tr>
		<?php
		}
	...
	<p>
		...
		</select> : sur les bases de données sélectionnées
	</p>
</form>
