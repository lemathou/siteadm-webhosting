...
<tr>
	...
	<td><a href="website.php?id=<?=$row["id"]?>"><?=$row["name"]?>.<?=$row["domain_name"]?></a></td>
	<td><?php echo $row["webmaster_email"]; ?></td>
	<td><?php if ($row["phppool_id"] && $phppool=phppool($row["phppool_id"])) echo $phppool->link(); ?></td>
</tr>
<?php
}
?>
...
{
?>
<p><select name="_website_action" onchange="if (this.value && confirm('Êtes-vous certain ?')) this.form.submit(); else this.selectedIndex = 0;">
